...
const DoughnutChart = memo(({ data, status }) => {
  ...
  // Adjust percentage calculation for specific statuses
                  ...
                  <Grid item xs={12} sm={6} md={3} key={status}>
                    ...
                    >
                      ...
                      {(status === 'manufactured' || status === 'transported') &&
                        ` (${(((component.statuses[status] || 0) / component.total) * 100).toFixed(
                          1
                        )}%)`}
                    </Typography>
...
const ProjectRowTab2 = memo(
  ({ project, isLoggedIn, onUpdateStatus, refreshData, userRole, onProjectSelect }) => {
    const [open, setOpen] = useState(false);
    const theme = useTheme();
    const isSmallScreen = useMediaQuery(theme.breakpoints.down('sm'));
          ...
          prevProjects.map((project) => ({
            ...
            components: project.components.map((component) => {
              if (component.id === componentId) {
                ...
                // Decrease fromStatus quantity except for 'manufactured'
                ...
                // Increase toStatus quantity
                updatedStatuses[toStatus] = (updatedStatuses[toStatus] || 0) + quantity;

                // Special case for 'manufactured' to 'transported'
