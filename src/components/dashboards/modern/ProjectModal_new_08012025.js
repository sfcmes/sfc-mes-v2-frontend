...
const SectionAccordion = memo(({ section, projectCode, onOpenDialog }) => {
  ...
  const calculateSectionStats = useMemo(() => {
    ...
    section.components.forEach(component => {
      if (!component?.history?.length) return;

      // Get latest status
      const latestHistory = component.history[0];
    ...
    Object.keys(counts).forEach(status => {
      const totalForPercentage = status === 'rejected' ? total : (total - counts.rejected);
      if (totalForPercentage > 0) {
        percentages[status] = ((counts[status] / totalForPercentage) * 100).toFixed(1);
      } else {
    ...
    return {
      ...
      percentages,
      total,
      totalRequired: total + counts.rejected // Total including rejected components
    };
  }, [section.components]);
      ...
      >
        <Box display="flex" alignItems="center" gap={1}>
          ...
          <Typography variant="subtitle1" sx={{ opacity: 0.9 }}>
            ({componentCount} ชิ้นงาน
            {(calculateSectionStats.counts?.rejected || 0) > 0 && 
              `, ต้องผลิตทั้งหมด ${calculateSectionStats.totalRequired} ชิ้น`}
            )
          </Typography>
        </Box>
        <Box display="flex" flexWrap="wrap" mt={1}>
          {statusOrder.map((status) => {
            const count = calculateSectionStats.counts?.[status] || 0;
            const percent = calculateSectionStats.percentages?.[status] || '0.0';
            // Check if count is greater than 0 to determine if we should display the chip
            return (
              <StatusChip 
                key={status} 
                status={status} 
                label={`${STATUS_THAI[status]}: ${count} (${percent}%)`} 
              />
            );
          })}
        </Box>
      ...
      <AccordionDetails>
        <Grid container spacing={0.5}>
          {Array.isArray(section.components) &&
            sortComponents(section.components).map(
              (component) =>
                component && (
                  <ComponentCard
                    key={component.id}
