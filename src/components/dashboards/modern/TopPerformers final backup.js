...
const SectionAccordion = memo(({ section, projectCode, setSelectedComponent }) => {
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant="subtitle1">{section.name || `ชั้นที่ ${section.id}`}</Typography>
...
const ProjectModal = memo(({ project, open, onClose, userRole, canEdit, onProjectSelect }) => {
  const theme = useTheme();
  ...
  const handleComponentUpdate = useCallback((updatedComponent) => {
    // Implementation of handleComponentUpdate
    // This should update the component in the project state
    console.log('Component updated:', updatedComponent);
    // You might want to update the project state here
  }, []);

  // เพิ่มฟังก์ชันการเรียงลำดับ
  const parseName = (name) => {
    const matchFRP = name.match(/^(\D+)-(\d+)-(\d+)$/);
    if (matchFRP) {
      const [, prefix, group, number] = matchFRP;
      return { prefix, group: parseInt(group, 10), number: parseInt(number, 10) };
    }
  ...
  // เรียงลำดับ sections และ components
  const sortedSections = useMemo(() => {
    if (!project || !project.sections) return [];
    return [...project.sections].sort((a, b) => {
      if (a.name && b.name) {
        return compareNames(a.name, b.name);
      }
      return a.id - b.id;
...
const ProjectRow = memo(
  ({
    ...
    }, [isSmallScreen, isMediumScreen, onRowClick, onProjectSelect, project]);
    ...
    // Helper function to parse names (can be used for both sections and components)
    const parseName = (name) => {
      // For FRP-XX-XX format
      const matchFRP = name.match(/^(\D+)-(\d+)-(\d+)$/);
      if (matchFRP) {
        const [, prefix, group, number] = matchFRP;
        return { prefix, group: parseInt(group, 10), number: parseInt(number, 10) };
      }
      // For XX-XX format (potential section naming)
      const matchSection = name.match(/^(\d+)-(\d+)$/);
      if (matchSection) {
        const [, group, number] = matchSection;
        return { group: parseInt(group, 10), number: parseInt(number, 10) };
      ...
      // For any other format, return the original name
      return { original: name };
    };
    ...
    const compareNames = (a, b) => {
      ...
      // If both have prefix, group, and number (FRP-XX-XX format)
      if (aInfo.prefix && bInfo.prefix) {
        if (aInfo.prefix !== bInfo.prefix) return aInfo.prefix.localeCompare(bInfo.prefix);
        if (aInfo.group !== bInfo.group) return aInfo.group - bInfo.group;
        return aInfo.number - bInfo.number;
      ...
      // If both have group and number (XX-XX format)
      if ('group' in aInfo && 'group' in bInfo) {
        if (aInfo.group !== bInfo.group) return aInfo.group - bInfo.group;
        return aInfo.number - bInfo.number;
      }

      // If both are just numbers
      if ('number' in aInfo && 'number' in bInfo && !('group' in aInfo) && !('group' in bInfo)) {
        return aInfo.number - bInfo.number;
      }

      // If formats are different or for any other case, use string comparison
      return a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' });
    };
    ...
    const sortComponents = (components) => {
      return components.sort((a, b) => {
        ...
        // Then sort by name using the same compareNames function
        return compareNames(a.name, b.name);
      });
    };

    const sortedSectionsWithSortedComponents = sortedSections.map((section) => ({
      ...section,
      components: sortComponents([...section.components]),
    }));
              ...
              <Collapse in={open} timeout="auto" unmountOnExit>
                ...
                >
                  ...
                  <Box sx={{ padding: theme.spacing(2) }}>
                    ...
                    <Table size="small">
                      <TableBody>
                        {sortedSectionsWithSortedComponents.map((section) => (
                          <SectionRow
                            key={section.id}
                            section={section}
                            projectCode={project.project_code}
