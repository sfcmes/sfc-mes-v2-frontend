...
const QRCodePage = () => {
  ...
  const filteredComponents = Array.isArray(components)
    ...
    // Custom sorting function for alphanumeric strings
    const naturalSort = (a, b) => {
      const splitIntoNumbersAndStrings = (str) => {
        return str.split(/(\d+)/).filter(Boolean).map(part => 
          /^\d+$/.test(part) ? parseInt(part, 10) : part.toLowerCase()
        );
      };
    
      const partsA = splitIntoNumbersAndStrings(a);
      const partsB = splitIntoNumbersAndStrings(b);
    ...
    const result = naturalSort(valueA, valueB);
                ...
                {sortedComponents.map((component) => {
                  ...
                  return (
                    <TableRow key={component.id}>
                      ...
                      <TableCell>{component.weight ? Number(component.weight).toFixed(3) : 'N/A'}</TableCell>
